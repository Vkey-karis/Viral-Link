...
const StrategyBoard: React.FC<Props> = ({ productData, onNext }) => {
  ...
  useEffect(() => {
    ...
    initScript();
  }, [duration, selectedTemplate, productData]);
                     ...
                     <div className="flex flex-wrap gap-2">
                        {voices.map((v) => (
                          <button
                            ...
                            className={`px-4 py-2 text-[10px] font-black uppercase tracking-widest rounded-xl border transition-all flex items-center gap-2 ${
                              selectedVoice === v.id
